...
def dilated_conv(in_channels, out_channels, kernel_size, stride, dilation, bias=True, pad='reflection'):
    if pad == 'reflection':
        ...
        conv = conv_with_dilation(in_channels, out_channels, kernel_size, stride, bias=bias,  pad='reflection', dilation=dilation, downsample_mode='stride')
    elif pad == 'zero':
        # padding = dilation
        # conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding=padding, dilation=dilation, bias=bias)
        conv = conv_with_dilation(in_channels, out_channels, kernel_size, stride, bias=bias, pad='zero',
                                  dilation=dilation, downsample_mode='stride')
    else:
...
def skip(
    ...
    need_sigmoid=False, need_bias=True,
    pad='reflection', upsample_mode='bilinear', downsample_mode='stride', act_fun='LeakyReLU',
    need1x1_up=True, dilations=[1,2,4,8,1]):  # Add dilations parameter
    assert len(num_channels_down) == len(num_channels_up) == len(num_channels_skip)
    n_scales = len(num_channels_down)
